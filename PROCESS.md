...
| Market rent (no source column) | when the source has **no market/asking-rent column**, every unit's market rent = the **max in-place rent of the same unit type** ("achievable market" proxy); supersedes the vacant-only rule above. Toggle `market_from_inplace` (default False; intake turns it on only when a market column is absent). |
